...
df_BAs = pd.read_csv('BAs_full.csv',header=0,index_col=0)
BAs = list(df_BAs['Name'])

BAs_ref = pd.read_excel('BAs_reference_table.xlsx',header=0)
name_mapping = dict(zip(BAs_ref['Name_geometry'], BAs_ref['Name_EIA']))     # Create a mapping from old BA names to standard EIA BA names

df = pd.read_csv('EIC.csv',header=0)
crs = {'init':'epsg:4326'}
# crs = {"init": "epsg:2163"}
...
joined['NAME'] = joined['NAME'].replace(name_mapping)   # Replace old BA names with standard EIA BA names in the combined['NAME'] column
